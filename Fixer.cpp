...
void Fixer::fix(const char* inFile,const char* outFile,int flags){
  setflags(0);
std:string tmpfile = outFile;
  tmpfile += '~';
  
  startFix(inFile,tmpfile.c_str());
  
  int pos=0;
  while(isWorking()){
    if(contFix(300)){
      slowFileCopy(tmpfile.c_str(),outFile);
    }
  ...
  if (remove(tmpfile.c_str()) != 0) {
   cout <<"failed to remove tmpfile: " << tmpfile.c_str() <<endl;
  }
